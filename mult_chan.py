...
class Application(tk.Frame):
    ...
    def _create_widgets(self):
        ...
        self.dat_paht = tk.Entry(self, width=20)
        self.dat_paht.pack(

#asksaveasfilename(initialdir = "/", 
#                  title = "Select file",
#                  filetypes = (("jpeg files","*.jpg"),("all files","*.*")))     
    ...
    def _update_canvas(self):
        ...
        self._ax.plot(t, np.sin(2*np.pi*t) + np.random.randn(t.shape[0])/10, 
                                label='ch0')
        self._ax.plot(t, np.cos(2*np.pi*t) + np.random.randn(t.shape[0])/10) 
        self._ax.plot(t, np.sin(2*np.pi*t)*np.cos(2*np.pi*t) + \
